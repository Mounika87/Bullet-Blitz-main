...
class Tutorial():
    def __init__(self, mainScreen, screenWidth, screenHeight, renderEngine, keyBindings, terrainList):
    ...
    def updateTutorial(self, eventList):
        ...
        if self.TutorialPaused:
            ...
            for event in eventList:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key in self.keyBindings.getMenuDefaultBindings()["esc"]:
                        self.TutorialPaused = False
            ...
            for event in eventList:
                ...
                # Event handler for Keyboard controls  
                if event.type == pygame.KEYDOWN:
                    if event.key in self.keyBindings.getGameCurrentBindings()["pause"]:
                        self.TutorialPaused = True
                    if not self.controlsDisabled:
                        if event.key in self.keyBindings.getGameCurrentBindings()["left"]: # keyboard button a is set for the left movement
                            self.playerMoveLeft = True
                        if event.key in self.keyBindings.getGameCurrentBindings()["right"]: # keyboard button b is set for the right movement
                ...
                if event.type == pygame.KEYUP:
                    if not self.controlsDisabled:
                        if event.key in self.keyBindings.getGameCurrentBindings()["left"]:
                            self.playerMoveLeft = False
                            self.counter = 0
                        if event.key in self.keyBindings.getGameCurrentBindings()["right"]: 
                            self.playerMoveRight = False
                            self.counter = 0
    ...
    def updateTutorialMenu(self):
        ...
        self.renderEngine.draw_text(self.renderEngine.getTextFromFile("tutorial", 1), self.extreme_font, (220,220,220), 240, 200, True, False)

        xPlacement = 370
        yPlacement = 480
        boxWidth = 380
        textOffset = 40 #- 30 * self.selectedLanguage TODO
        ...
        self.renderEngine.draw_text(self.renderEngine.getTextFromFile("tutorial", 2), self.large_font, (255,255,255), xPlacement+textOffset, 490, False, False)
        self.renderEngine.draw_text(self.renderEngine.getTextFromFile("tutorial", 3), self.large_font, (255,255,255), xPlacement+textOffset, 560, False, False)
    ...
    def tutorialFirstSection(self):
        ...
        for index in (5,7,9,11,13):
            ...
            if(self.TuturialInternalClock > StartTime and 
                ...
                self.renderEngine.draw_text(self.renderEngine.getTextFromFile("tutorial", index), self.large_font, (220,220,220), 500, self.firstLineY, True, True)
                if(self.TuturialInternalClock > SecondText):
                    self.renderEngine.draw_text(self.renderEngine.getTextFromFile("tutorial", index+1), self.large_font, (220,220,220), 500, self.secondLineY, True, True)
